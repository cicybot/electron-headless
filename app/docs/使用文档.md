# Electron Headless Browser 使用文档

## 目录

1. [简介](#简介)
2. [系统要求](#系统要求)
3. [安装与配置](#安装与配置)
4. [快速开始](#快速开始)
5. [账户管理](#账户管理)
6. [Web 控制台界面使用](#web-控制台界面使用)
7. [API 使用指南](#api-使用指南)
8. [MCP 服务器集成](#mcp-服务器集成)
9. [常见使用场景](#常见使用场景)
10. [高级功能](#高级功能)
11. [故障排除](#故障排除)
12. [开发指南](#开发指南)

## 简介

Electron Headless Browser 是一个基于 Electron 的无头浏览器自动化工具，提供完整的浏览器控制能力，包括窗口管理、JavaScript 执行、截图、网络监控等功能。通过 HTTP API 和 Web 界面，您可以远程控制多个浏览器实例，实现网页自动化、数据抓取、UI 测试等任务。

### 主要特性

- 🚀 基于 Electron，完整的 Chromium 浏览器支持
- 🖥️ 多窗口并发管理
- 📸 实时截图和录制
- 🔧 JavaScript 代码执行
- 🍪 Cookie 管理和会话隔离
- 🌐 网络请求监控
- 🛡️ 代理服务器支持
- 📱 响应式界面设计

## 系统要求

- **操作系统**: macOS, Windows, Linux
- **Node.js**: v16.0 或更高版本
- **Electron**: 全局安装 (npm install -g electron)
- **内存**: 至少 2GB RAM
- **磁盘空间**: 至少 500MB 可用空间

## 安装与配置

### 方式一：从源码安装

```bash
# 1. 克隆项目
git clone <repository-url>
cd electron-mcp

# 2. 全局安装 Electron
npm install -g electron

# 3. 安装后端依赖
cd app
npm install

# 4. 安装前端依赖
cd ../render
npm install

# 5. 返回项目根目录
cd ..

# 6. 构建项目
./build.sh
```

### 方式二：使用 Docker

```bash
# 全局安装 Electron (如果需要本地开发)
npm install -g electron

# 构建 Docker 镜像
docker build -t electron-mcp .

# 运行容器
docker run --name electron-mcp \
  -p 3456:3456 \
  -v $(pwd)/assets:/home/electron/.config/electron-mcp \
  electron-mcp
```

### 环境配置

创建 `.env` 文件配置环境变量：

```env
# 服务器端口 (默认: 3456)
PORT=3456

# 应用名称
APP_NAME=MyAutomation

# 日志级别
LOG_LEVEL=info

# 资源存储目录
ASSETS_DIR=/home/user/assets
```

## 快速开始

### 启动服务

```bash
# 开发模式 (推荐用于开发)
cd app && npm run dev

# 生产模式
cd app && npm run prod

# 或使用构建版本
cd app && npm start
```

服务启动后，您可以通过以下方式访问：

- **Web 控制台**: http://127.0.0.1:3456 (如果前端已构建)
- **API 端点**: http://127.0.0.1:3456/rpc
- **MCP 端点**: http://127.0.0.1:3456/mcp (Model Context Protocol)

### 创建第一个窗口

使用 curl 命令创建浏览器窗口：

```bash
curl -X POST http://127.0.0.1:3456/rpc \
  -H "Content-Type: application/json" \
  -d '{
    "method": "openWindow",
    "params": {
      "account_index": 1,
      "url": "https://www.google.com"
    }
  }'
```

响应示例：
```json
{
  "ok": true,
  "result": {
    "id": 1
  }
}
```

### 获取截图

```bash
curl "http://127.0.0.1:3456/screenshot?id=1" \
  --output screenshot.png
```

## 账户管理

### 账户概念

Electron Headless Browser 支持多账户隔离，每个账户都有独立的浏览器上下文：

- **账户索引**: 整数标识符，用于区分不同的浏览器上下文
- **沙盒隔离**: 不同账户的窗口完全隔离，不共享任何数据
- **资源共享**: 同一账户的窗口在相同域名下共享cookies和缓存
- **默认账户**: 如果不指定`account_index`，默认使用账户0

### 账户隔离示例

```bash
# 创建账户1的窗口
curl -X POST http://127.0.0.1:3456/rpc \
  -H "Content-Type: application/json" \
  -d '{
    "method": "openWindow",
    "params": {
      "account_index": 1,
      "url": "https://example.com"
    }
  }'

# 创建账户2的窗口（完全隔离）
curl -X POST http://127.0.0.1:3456/rpc \
  -H "Content-Type: application/json" \
  -d '{
    "method": "openWindow",
    "params": {
      "account_index": 2,
      "url": "https://example.com"
    }
  }'
```

### 窗口管理

所有操作都需要指定`win_id`（窗口ID）：

1. 使用`list_pages`或`getWindows`获取所有活跃窗口及其账户信息
2. 从结果中选择目标窗口的`win_id`
3. 在后续API调用中传递该`win_id`

#### 获取窗口列表

```bash
curl -X POST http://127.0.0.1:3456/rpc \
  -H "Content-Type: application/json" \
  -d '{
    "method": "getWindows"
  }'
```

响应示例：
```json
{
  "ok": true,
  "result": {
    "1": {
      "https://example.com": {
        "id": 1,
        "wcId": 1,
        "win": "[BrowserWindow]"
      }
    },
    "2": {
      "https://example.com": {
        "id": 2,
        "wcId": 2,
        "win": "[BrowserWindow]"
      }
    }
  }
}
```

### 最佳实践

- 为不同用户或会话使用不同的`account_index`
- 在自动化任务开始时创建专用账户
- 使用账户隔离来避免会话间的干扰
- 定期清理不需要的账户和窗口

## Web 控制台界面使用

### 界面概述

Web 控制台提供直观的图形界面来管理浏览器窗口：

1. **窗口列表**: 显示所有活跃的浏览器窗口缩略图
2. **窗口详情**: 每个窗口的详细控制面板
3. **控制台**: JavaScript 代码执行
4. **网络监控**: HTTP 请求和响应日志

### 创建新窗口

1. 在主界面点击 "新建窗口"
2. 输入目标 URL（例如：`https://www.example.com`）
3. 选择账户索引（用于会话隔离）
4. 点击 "创建"

### 窗口操作

#### 基本导航

- **URL 输入框**: 输入新 URL 并按回车导航
- **刷新按钮**: 重新加载当前页面
- **后退按钮**: 返回上一页

#### JavaScript 执行

在控制台标签页中：

1. 在代码编辑器中输入 JavaScript 代码
2. 点击 "运行" 按钮执行
3. 查看输出结果

示例代码：
```javascript
// 获取页面标题
(() => {
  return document.title;
})()

// 点击元素
(() => {
  const button = document.querySelector('button.primary');
  if (button) {
    button.click();
    return 'Button clicked';
  }
  return 'Button not found';
})()
```

#### 网络监控

网络标签页显示：

- 请求方法 (GET, POST, etc.)
- 请求 URL
- 状态码
- 时间戳
- 请求/响应头（点击展开）

### 高级功能

#### 自动刷新

- 勾选 "Auto-Refresh" 复选框启用自动截图刷新
- 适用于监控动态内容变化

#### Cookie 管理

```javascript
// 导出 cookies
await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'exportCookies',
    params: { win_id: 1 }
  })
});

// 导入 cookies
await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'importCookies',
    params: {
      win_id: 1,
      cookies: [...]
    }
  })
});
```

## API 使用指南

### 基础概念

- **窗口 ID**: 每个浏览器窗口的唯一标识符
- **账户索引**: 用于会话隔离的整数标识
- **RPC 调用**: 通过 HTTP POST 到 `/rpc` 端点执行操作

### 核心 API 方法

#### 窗口管理

**创建窗口**
```javascript
POST /rpc
{
  "method": "openWindow",
  "params": {
    "account_index": 1,
    "url": "https://example.com",
    "others": {
      "userAgent": "Custom Bot 1.0",
      "openDevtools": false,
      "proxy": "socks5://127.0.0.1:9050"
    }
  }
}
```

**获取窗口列表**
```javascript
POST /rpc
{
  "method": "getWindows"
}
```

**关闭窗口**
```javascript
POST /rpc
{
  "method": "closeWindow",
  "params": {
    "win_id": 1
  }
}
```

#### 页面操作

**导航到 URL**
```javascript
POST /rpc
{
  "method": "loadURL",
  "params": {
    "win_id": 1,
    "url": "https://new-url.com"
  }
}
```

**执行 JavaScript**
```javascript
POST /rpc
{
  "method": "executeJavaScript",
  "params": {
    "win_id": 1,
    "code": "(() => { return document.title; })()"
  }
}
```

**获取截图**
```javascript
GET /screenshot?id=1
```

#### 会话管理

**设置 User Agent**
```javascript
POST /rpc
{
  "method": "setUserAgent",
  "params": {
    "win_id": 1,
    "userAgent": "Mozilla/5.0 (Custom Bot)"
  }
}
```

**Cookie 操作**
```javascript
// 导入 cookies
POST /rpc
{
  "method": "importCookies",
  "params": {
    "win_id": 1,
    "cookies": [{
      "name": "session_id",
      "value": "abc123",
      "domain": ".example.com"
    }]
  }
}

// 导出 cookies
POST /rpc
{
  "method": "exportCookies",
  "params": {
    "win_id": 1
  }
}
```

### 错误处理

所有 API 调用返回统一的响应格式：

```json
{
  "ok": true|false,
  "result": <data>,
  "error": "<error_message>"
}
```

错误示例：
```json
{
  "ok": false,
  "error": "Window with id 999 not found"
}
```

## MCP 服务器集成

### MCP 简介

Model Context Protocol (MCP) 是一个用于连接AI模型到工具和数据源的协议。Electron Headless Browser 提供了 MCP 服务器，支持 Playwright 风格的浏览器自动化工具。

### MCP 端点

- **URL**: `http://127.0.0.1:3456/mcp`
- **协议**: JSON-RPC 2.0 over HTTP
- **工具**: 提供完整的浏览器自动化工具集

### 可用工具

#### 导航工具
- `new_page`: 创建新浏览器窗口
- `close_page`: 关闭浏览器窗口
- `navigate_page`: 导航到指定URL
- `list_pages`: 列出所有活跃窗口
- `switch_account`: 切换账户上下文

#### 输入自动化工具
- `click`: 点击指定坐标
- `fill`: 填写表单字段
- `press_key`: 按下键盘按键
- `fill_form`: 填写完整表单

#### 调试工具
- `evaluate_script`: 执行JavaScript代码
- `take_screenshot`: 截取窗口截图
- `take_snapshot`: 获取页面快照

#### 网络监控工具
- `get_network_request`: 获取网络请求详情
- `list_network_requests`: 列出所有网络请求

#### 性能工具
- `performance_start_trace`: 开始性能追踪
- `performance_stop_trace`: 停止性能追踪
- `performance_analyze_insight`: 分析性能数据

#### 其他工具
- `wait_for`: 等待元素或条件
- `emulate`: 设备模拟
- `resize_page`: 调整页面大小

### MCP 配置

创建 `mcp-config.json` 文件：

```json
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "electron-mcp-ptools": {
      "type": "local",
      "command": ["curl", "-X", "POST", "http://127.0.0.1:3456/mcp", "-H", "Content-Type: application/json", "-d", "@-"],
      "description": "Playwright风格的浏览器自动化工具"
    }
  }
}
```

### MCP 工具使用示例

#### 创建新页面
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "tools/call",
  "params": {
    "name": "new_page",
    "arguments": {
      "url": "https://example.com",
      "account_index": 1
    }
  }
}
```

#### 点击操作
```json
{
  "jsonrpc": "2.0",
  "id": 2,
  "method": "tools/call",
  "params": {
    "name": "click",
    "arguments": {
      "win_id": 1,
      "x": 100,
      "y": 200
    }
  }
}
```

#### 填写表单
```json
{
  "jsonrpc": "2.0",
  "id": 3,
  "method": "tools/call",
  "params": {
    "name": "fill",
    "arguments": {
      "win_id": 1,
      "selector": "#username",
      "value": "testuser"
    }
  }
}
```

### MCP 与传统 API 的区别

- **MCP**: 结构化的工具调用，AI 友好的接口
- **传统 API**: 直接的 HTTP 调用，更适合程序化使用
- **兼容性**: 两个接口可以同时使用，共享相同的底层功能

### 账户感知

所有 MCP 工具都支持账户上下文：

- 工具调用时可以指定 `account_index` 进行账户验证
- 同一会话可以维护活动账户状态
- 自动防止跨账户的意外操作

## 常见使用场景

### 1. 网页数据抓取

```javascript
const puppeteer = require('puppeteer'); // 模拟调用

async function scrapeData(url) {
  // 1. 创建窗口
  const createResponse = await fetch('/rpc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      method: 'openWindow',
      params: { url }
    })
  });
  const { result: { id: winId } } = await createResponse.json();

  // 2. 等待页面加载
  await new Promise(resolve => setTimeout(resolve, 2000));

  // 3. 提取数据
  const dataResponse = await fetch('/rpc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      method: 'executeJavaScript',
      params: {
        win_id: winId,
        code: `(() => {
          const titles = Array.from(document.querySelectorAll('h2'))
            .map(h2 => h2.textContent);
          return titles;
        })()`
      }
    })
  });

  const { result: titles } = await dataResponse.json();
  return titles;
}
```

### 2. 自动化表单填写

```javascript
async function fillForm(winId, formData) {
  const code = `
(() => {
  try {
    // 填写表单字段
    document.querySelector('#username').value = '${formData.username}';
    document.querySelector('#password').value = '${formData.password}';

    // 提交表单
    document.querySelector('form').submit();

    return 'Form submitted successfully';
  } catch (error) {
    return 'Error: ' + error.message;
  }
})()
`;

  const response = await fetch('/rpc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      method: 'executeJavaScript',
      params: { win_id: winId, code }
    })
  });

  return response.json();
}
```

### 3. 截图和监控

```javascript
async function monitorPage(winId, interval = 5000) {
  const screenshots = [];

  while (true) {
    // 获取截图
    const response = await fetch(\`/screenshot?id=\${winId}\`);
    const buffer = await response.arrayBuffer();
    screenshots.push(buffer);

    // 检查页面状态
    const statusResponse = await fetch('/rpc', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        method: 'executeJavaScript',
        params: {
          win_id: winId,
          code: '(() => ({ url: window.location.href, title: document.title }))()'
        }
      })
    });

    const { result: status } = await statusResponse.json();
    console.log('Page status:', status);

    await new Promise(resolve => setTimeout(resolve, interval));
  }
}
```

### 4. 多账户自动化

```javascript
async function createIsolatedSessions(urls) {
  const sessions = [];

  for (let i = 0; i < urls.length; i++) {
    const response = await fetch('/rpc', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        method: 'openWindow',
        params: {
          account_index: i + 1, // 不同的账户索引实现会话隔离
          url: urls[i],
          others: {
            userAgent: \`Bot-\${i + 1}/1.0\`,
            proxy: getRandomProxy() // 为每个会话分配不同代理
          }
        }
      })
    });

    const { result } = await response.json();
    sessions.push(result);
  }

  return sessions;
}
```

## 高级功能

### 代理支持

为窗口配置代理服务器：

```javascript
const response = await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'openWindow',
    params: {
      account_index: 1,
      url: 'https://checkip.amazonaws.com',
      others: {
        proxy: 'socks5://127.0.0.1:9050' // Tor 代理
      }
    }
  })
});
```

支持的代理格式：
- `http://proxy.example.com:8080`
- `socks4://127.0.0.1:1080`
- `socks5://user:pass@proxy.example.com:9050`

### 网络监控

监控所有 HTTP 请求：

```javascript
// 获取网络日志
const response = await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'getRequests'
  })
});

const { result: requests } = await response.json();

// 分析请求
requests.forEach(req => {
  console.log(\`\${req.method} \${req.url} -> \${req.statusCode}\`);
});
```

### 设备模拟

模拟移动设备：

```javascript
const mobileConfig = {
  userAgent: 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_7_1 like Mac OS X) AppleWebKit/605.1.15',
  viewport: { width: 375, height: 667 }
};

// 通过 JavaScript 设置视口
await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'executeJavaScript',
    params: {
      win_id: 1,
      code: \`
(() => {
  // 模拟移动设备视口
  Object.defineProperty(window, 'innerWidth', { value: 375 });
  Object.defineProperty(window, 'innerHeight', { value: 667 });
  window.dispatchEvent(new Event('resize'));
  return 'Mobile viewport set';
})()
\`
    }
  })
});
```

## 故障排除

### 常见问题

#### 1. 连接失败

**问题**: 无法连接到服务器
**解决方案**:
- 检查服务是否正在运行: `curl http://127.0.0.1:3456/rpc -d '{"method":"ping"}'`
- 检查防火墙设置
- 确认端口 3456 未被占用

#### 2. 窗口创建失败

**问题**: `openWindow` 返回错误
**解决方案**:
- 检查 URL 格式是否正确
- 确认账户索引为非负整数
- 检查磁盘空间和内存使用情况

#### 3. JavaScript 执行失败

**问题**: `executeJavaScript` 返回错误
**解决方案**:
- 使用 IIFE 包装代码: `(() => { /* code */ })()`
- 检查 DOM 元素是否存在
- 添加错误处理:

```javascript
code: \`
(() => {
  try {
    // 您的代码
    return result;
  } catch (error) {
    return 'Error: ' + error.message;
  }
})()
\`
```

#### 4. 截图为空或失败

**问题**: 截图返回空白图像
**解决方案**:
- 等待页面完全加载: `await new Promise(resolve => setTimeout(resolve, 2000))`
- 检查窗口是否可见
- 确认窗口 ID 正确

### 调试技巧

#### 启用开发者工具

```javascript
// 打开 DevTools
await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'openDevTools',
    params: { win_id: 1 }
  })
});
```

#### 查看系统信息

```javascript
// 获取系统和进程信息
const response = await fetch('/rpc', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    method: 'info'
  })
});

const { result: systemInfo } = await response.json();
console.log('System Info:', systemInfo);
```

#### 日志记录

```javascript
// 在 JavaScript 代码中添加日志
code: \`
(() => {
  console.log('Script started');
  const result = /* 您的操作 */;
  console.log('Result:', result);
  return result;
})()
\`
```

## 开发指南

### 项目结构

```
electron-mcp/
├── app/                    # 后端 (Node.js/Electron)
│   ├── src/
│   │   ├── main.js        # Electron 主进程
│   │   ├── utils.js       # 工具函数
│   │   └── db/            # 数据库相关
│   └── tests/             # 后端测试
├── render/                 # 前端 (React/TypeScript)
│   └── src/
│       ├── components/    # React 组件
│       ├── types.ts       # TypeScript 类型定义
│       └── App.tsx        # 主应用组件
├── chrome-extension/      # Chrome 扩展 (可选)
└── build.sh              # 构建脚本
```

### 贡献指南

1. **代码规范**: 遵循项目中的 `AGENTS.md` 指南
2. **测试**: 为新功能添加单元测试
3. **文档**: 更新相关文档
4. **提交**: 使用有意义的提交信息

### 扩展开发

#### 添加新的 RPC 方法

在 `app/src/main.js` 中：

```javascript
// 添加新方法
case 'yourMethod':
  result = await yourMethodImplementation(params);
  break;
```

#### 创建新的前端组件

在 `render/src/components/` 中创建组件：

```tsx
import React from 'react';

interface YourComponentProps {
  // 属性定义
}

export const YourComponent: React.FC<YourComponentProps> = ({ /* props */ }) => {
  return (
    <div>
      {/* 组件内容 */}
    </div>
  );
};
```

### 性能优化

- 使用连接池管理数据库连接
- 实现缓存机制减少重复操作
- 监控内存使用情况
- 使用流式处理大文件

---

## 技术支持

如果您在使用过程中遇到问题，请：

1. 查看[故障排除](#故障排除)章节
2. 检查[常见问题](https://github.com/your-repo/issues)
3. [提交问题报告](https://github.com/your-repo/issues/new)

## 许可证

本项目采用 MIT 许可证。</content>
<parameter name="filePath">/Users/data/electron/electron-mcp/使用文档.md